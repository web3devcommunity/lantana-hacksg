...
async function getBundlr() {
  const signer = (await fetchSigner()) ?? console.log('Cannot get signer');
  const provider = signer?.provider ?? console.log('Cannot get provider');
  ...
  const balance = await bundlr.getBalance(
    (await signer?.getAddress()) ?? 'Cannot get address',
  );
