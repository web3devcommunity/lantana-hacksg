...
export const CauseAttestationList = ({
  causeAttestations,
}: {
  causeAttestations: CauseAttestation[];
}) => {
  return (
    <div>
      <section>
        ...
        >
          {causeAttestations
            .filter((cA) => cA.isProfessional)
            .map((causeAttestation: CauseAttestation) => {
              return (
                <>
                  <ListItem alignItems="flex-start">
                    <ListItemAvatar>
                      <Badge
                        ...
                        badgeContent={<SmallAvatar src="/badge_climate.png" />}
                      >
                        <Avatar
                          alt="Travis Howard"
                          src={getAvatarUrl()}
        ...
        <Grid container alignItems="flex-start">
          <Grid item>
            <AvatarGroup max={6} total={24}>
              {causeAttestations
                .filter((cA) => !cA.isProfessional)
                .map((causeAttestation: CauseAttestation, i: number) => {
                  return (
                    <Avatar
                      key={i}
